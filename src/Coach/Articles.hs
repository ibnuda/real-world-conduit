...
getArticlesCoach authres mtag mauthor mfavorited mlimit moffset = do
  articles <- runDb $ selectArticles (userUsername <$> authresToMaybe authres)
                                     False -- is feed?
                                     Nothing -- slug name?
                                     mtag
                                     mauthor
                                     ...
                                     (fromMaybe 20 mlimit)
                                     (fromMaybe 0 moffset)
  return $ ResponseMultiArticle (map resultQueryToResponseArticle articles)
                                (length articles)

getArticleSlugCoach
  :: MonadIO m => AuthResult User -> Text -> CoachT m ResponseArticle
...
getArticlesFeed (Authenticated User {..}) mlimit moffset = do
  articles <- runDb $ selectArticles (Just userUsername)
                                     True
                                     Nothing
                                     Nothing
                                     Nothing
...
postArticleCreateCoach (Authenticated User {..}) (RequestCreateArticle RequestCreateArticleBody {..})
  = do
    let slug = toSlug reqcrtarticlTitle
    articles <- runDb $ do
      insertArticle userUsername
                    slug
                    reqcrtarticlTitle
                    reqcrtarticlDescription
                    reqcrtarticlBody
      upsertMaybeTags reqcrtarticlTagList slug
      selectArticles (Just userUsername)
                     False
                     (Just slug)
                     Nothing
                     Nothing
...
deleteArticleSlugCoach (Authenticated User {..}) slug = do
  ...
  when (null users) $ throwError err401
...
putArticleSlugCoach
  ...
  -> CoachT m ResponseArticle
putArticleSlugCoach (Authenticated User {..}) slug (RequestUpdateArticle req@RequestUpdateArticleBody {..})
  = do
    users <- runDb $ isArticleAuthor userUsername slug
    when (null users) $ throwError err401
      { errBody = encodeRespError "Not the author or article doesn't exist."
...
getCommentsSlugCoach authres slug = do
  ...
  comments <- runDb
    $ selectComments (userUsername <$> authresToMaybe authres) slug
...
postCommentSlugCoach (Authenticated User {..}) slug (RequestComment (RequestCommentBody body))
  = do
    marticle <- runDb $ getBy $ UniqueSlug slug
    when (isNothing marticle) $ throwError err404
      { errBody = encodeRespError "There's no such thing."
      }
    ...
    case mcomment of
      ...
      Nothing ->
        throwError err410 { errBody = encodeRespError "Now it's gone." }
postCommentSlugCoach _ _ _ =
  throwError err401 { errBody = encodeRespError "Only authorised something." }

deleteCommentSlugIdCoach
  :: MonadIO m => AuthResult User -> Text -> Int64 -> CoachT m NoContent
deleteCommentSlugIdCoach (Authenticated User {..}) slug id = do
  comments <- runDb $ selectCommentByUsernameSlugId userUsername slug $ toSqlKey
    id
  when (null comments) $ throwError err401
...
deleteFavoriteArticleCoach
  :: MonadIO m => AuthResult User -> Text -> CoachT m ResponseArticle
deleteFavoriteArticleCoach (Authenticated User {..}) slug = do
  favs <- runDb $ isFavoritingArticle userUsername slug
  when (null favs) $ throwError err401
    { errBody = encodeRespError
...
resultQueryToResponseArticle (entarticle, entauthor, vtags, vfavcounts, vfaving, vfoll)
  = let Article {..} = entityVal entarticle
    ...
    in  ResponseArticleBody articleSlug
                            articleTitle
                            articleDescription
                            articleBody
                            tagnames
